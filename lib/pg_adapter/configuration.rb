...
module PgAdapter
  ...
  def self.failover_patch?
    PgAdapter.configuration.add_failover_patch || false
  end

  def self.reconnect_with_backoff?
    !PgAdapter.configuration.reconnect_with_backoff.empty?
  end

  def self.reset_column_information_patch?
    PgAdapter.configuration.add_reset_column_information_patch || false
  end
